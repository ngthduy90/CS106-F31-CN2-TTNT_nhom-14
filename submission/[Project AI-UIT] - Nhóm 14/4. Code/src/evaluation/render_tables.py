...
def _no_dash(text: str) -> str:
    """Bỏ dấu gạch dài khỏi mọi nhãn đọc từ file kết quả.

    Văn bản của dự án thống nhất không dùng ký tự này. File kết quả sinh trước khi thống
    nhất vẫn còn, nên bước sinh bảng dọn tại chỗ thay vì bắt chạy lại thí nghiệm.
    """
...
def render_e1(payload: dict) -> str:
    ...
    # Khoảng cách tới baseline "môi giới" là con số đáng nhớ nhất của bảng, và nó cũng
    # xuất hiện ở slide lẫn README. Sinh ra ở đây để cả ba tài liệu dùng chung một giá
    # trị thay vì mỗi chỗ tự tính lại rồi làm tròn khác nhau.
...
def render_e1_slide(payload: dict) -> str:
    ...
    for name in SLIDE_MODELS:
        ...
        lines.append(
            ...
            f"{_vi(model['cv_mean']['RMSE (tỷ)'], 3)} | {_vi(model['cv_mean']['R²'], 3)} |"
        )
    ...
    if baseline:
        ...
        lines += [
            "",
            f"{who} hơn baseline môi giới {_vi(gap, 2)} điểm phần trăm MdAPE.",
        ]
        if len(leaders) > 1:
            lines.append(
