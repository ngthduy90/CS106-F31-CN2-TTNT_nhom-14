...
def summary(node) -> str:
    """Dòng đầu của docstring, tức phần trả lời "hàm này làm gì".

    Dấu gạch dài trong docstring được quy về dấu chấm giữa: docstring là chú thích mã
    nguồn, nhưng bảng sinh ra từ nó là một tài liệu nộp, và tài liệu nộp của dự án không
    dùng ký tự đó.
    """
    text = ast.get_docstring(node) or ""
    first = text.strip().split("\n\n")[0].replace("\n", " ").strip()
    cleaned = " ".join(first.split()).replace(" — ", " · ").replace("—", "·")
    return cleaned or "_(chưa có mô tả)_"
